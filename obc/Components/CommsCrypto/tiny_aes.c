 ...
 * Tiny-AES-128-CBC Implementation Source
 * ============================================================================
 */
...
void TinyAes_EncryptCbc(TinyAesContext* ctx, uint8_t* buf, size_t length) {
    ...
    for (size_t i = 0; i < length; i += AES_BLOCKLEN) {
        ...
        for (size_t b = 0; b < AES_BLOCKLEN; ++b) {
            buf[i + b] ^= iv[b];
        }
...
void TinyAes_DecryptCbc(TinyAesContext* ctx, uint8_t* buf, size_t length) {
    ...
    for (size_t i = 0; i < length; i += AES_BLOCKLEN) {
        memcpy(nextIv, buf + i, AES_BLOCKLEN);
        InvCipher((state_t*)(buf + i), ctx->roundKey);
        for (size_t b = 0; b < AES_BLOCKLEN; ++b) {
            buf[i + b] ^= ctx->iv[b];
        }
        memcpy(ctx->iv, nextIv, AES_BLOCKLEN);
    }
}

size_t TinyAes_ApplyPkcs7Padding(uint8_t* buf, size_t dataLen, size_t maxBufLen) {
    uint8_t padVal = (uint8_t)(AES_BLOCKLEN - (dataLen % AES_BLOCKLEN));
    if (padVal == 0) {
        padVal = AES_BLOCKLEN;
    }
    size_t totalLen = dataLen + padVal;
    if (totalLen > maxBufLen) {
        return 0; // Buffer insufficient
    }
    for (size_t i = dataLen; i < totalLen; ++i) {
        buf[i] = padVal;
    }
    return totalLen;
}
...
size_t TinyAes_RemovePkcs7Padding(const uint8_t* buf, size_t paddedLen) {
    if (paddedLen == 0 || (paddedLen % AES_BLOCKLEN) != 0) {
        return 0;
    }
    uint8_t padVal = buf[paddedLen - 1];
    if (padVal == 0 || padVal > AES_BLOCKLEN) {
        return 0; // Invalid padding byte
    }
    for (size_t i = paddedLen - padVal; i < paddedLen; ++i) {
        if (buf[i] != padVal) {
            return 0; // Corrupt padding sequence
        }
    }
    return paddedLen - padVal;
}
